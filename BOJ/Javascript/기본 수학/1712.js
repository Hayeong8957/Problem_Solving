 ...
 *
 *
 * 풀이 : A = 고정 비용, B = 가변 비용, C = 판매 가격, x = 판매량
 * 1) 판매 누적 금액 > A + B ===> 손익분기점을 넘어감
 *    A + Bx < Cx
 *    = A < Cx - Bx
 *    = A < (C-B)x
 *    ===> C-B가 0이하(즉, 0 또는 음수)로 떨어지면 손익분기점은 없으며, 계속 적자다.
 * 2) '-1'을 출력하는 조건문
 *  2-1) if(C-B <= 0) { console.log(-1); }
 *      A < (C-B)x 식을 생각해서 C-B가 0이하면 손익분기점은 없다고 표현할 수 있다.
 *  2-2) if(C <= B) { console.log(-1); }
 *      C-B > 0이 돼야 하므로, C <= B으로 정리해주면 손익분기점이 없다고 표현할 수 있다.
 */
...
if (margin <= 0) {
  ...
  console.log(Math.floor(A / margin) + 1);
}
