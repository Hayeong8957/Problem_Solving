 ...
 * 1203
 *
 * 풀이 : 01호실부터 채움, 모든 층수 다 채워져야 02호에 손님 보냄 => 층수가 배수의 숫자가 됨
 * 1. N번째 손님에서 N은 입력되어온 층수로 나누고 나머지를 구하는 대상이 된다.
 * 1-1) 값은 호수번호(나누어 떨어지지 않고 소수로 나오면 무조건 올린 숫자)
 * 1-2) 나머지는 층수번호
 * 1-3) 딱 나눠져서 나머지가 0이 나오면 H의 값 그대로 출력값으로 써야함.
 *
 * 2. 조건문으로 나눠서 10이하 호수의 방은 0을 붙여서 출력조건에 만족
 * ex) 01호, 02호, 03호, 04호, ..., 09호, 10호
 *
 *
 *  */
...
const A = parseInt(input.shift()); // 첫번째 input개수 나타내는 값 제거
